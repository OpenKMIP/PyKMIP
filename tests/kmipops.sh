...
function run_kmip_test
{
    ...
    #debug
    echo kmip_script=${kmip_script}
    echo kmip_params=${kmip_params}
    ...
    #${pythonexe} ${script_path}/${1}.py -c ${config_session} ${2} 2>&1 | tee ${outfile}
...
function test_sym_key
{
    ...
    do
        ...
        do
            ...
            do
                if [ "${p}" == "NONE" ]; then
                    tst_msg="A message that needs no padding."
...
function test_sym_encdec
{
    ...
    #debug
    echo decrypt3 "-i ${1} -m b'${cipher_msg}' -d ${3} -a ${4}"
    
    run_kmip_test decrypt3 "-i ${1} -m b'${cipher_msg}' -d ${3} -a ${4}"
