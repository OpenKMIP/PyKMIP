...
class TestProxyKmipClientIntegration(testtools.TestCase):
    ...
    def test_symmetric_key_create_get_destroy(self):
        ...
        try:
            ...
            self.assertRaises(exceptions.KmipOperationFailure, self.client.get, uid)
            self.assertRaises(exceptions.KmipOperationFailure, self.client.destroy, uid)
    ...
    def test_create_get_wrapped_destroy(self):
        ...
        self.client.revoke(
            enums.RevocationReasonCode.CESSATION_OF_OPERATION, wrapping_id
        )
    ...
    def test_register_wrapped_get_destroy(self):
        ...
        self.assertEqual(
            enums.WrappingMethod.ENCRYPT, key_wrapping_data.get("wrapping_method")
        )
        ...
        self.assertEqual(
            enums.BlockCipherMode.NIST_KEY_WRAP, cp.get("block_cipher_mode")
        )
        self.assertEqual(
    ...
    def test_asymmetric_key_pair_create_get_destroy(self):
        ...
        try:
            ...
            self.assertEqual(
                public_key.cryptographic_algorithm, enums.CryptographicAlgorithm.RSA
            )
            ...
            self.assertEqual(
                private_key.cryptographic_algorithm, enums.CryptographicAlgorithm.RSA
            )
    ...
    def test_derive_key_using_pbkdf2(self):
        ...
        self.assertEqual(
            enums.CryptographicAlgorithm.AES, attribute.attribute_value.value
        )
    ...
    def test_create_key_pair_sign_signature_verify(self):
        ...
        # Clean up.
        self.client.revoke(
            enums.RevocationReasonCode.CESSATION_OF_OPERATION, public_key_id
        )
        self.client.revoke(
            enums.RevocationReasonCode.CESSATION_OF_OPERATION, private_key_id
        )
    ...
    def test_certificate_register_locate_destroy(self):
        ...
        certificate = objects.Certificate(
            enums.CertificateType.X_509, value, masks=usage_mask, name=label
        )
    ...
    def test_create_getattributes_locate_destroy(self):
        ...
        result_id, result_attributes = self.client.get_attributes(
            uid=a_id, attribute_names=["Initial Date"]
        )
        ...
        result_id, result_attributes = self.client.get_attributes(
            uid=b_id, attribute_names=["Initial Date"]
        )
        self.assertEqual(1, len(result_attributes))
        self.assertEqual("Initial Date", result_attributes[0].attribute_name.value)
        ...
        result = self.client.locate(
            attributes=[
                self.attribute_factory.create_attribute(
                    enums.AttributeType.UNIQUE_IDENTIFIER, a_id
                )
            ]
        )
        self.assertEqual(1, len(result))
        ...
        result = self.client.locate(
            attributes=[
                self.attribute_factory.create_attribute(
                    enums.AttributeType.UNIQUE_IDENTIFIER, b_id
                )
            ]
        )
        self.assertEqual(1, len(result))
    ...
    def test_split_key_register_get_destroy(self):
        ...
        try:
            ...
            self.assertEqual(
                enums.CryptographicAlgorithm.AES, result.cryptographic_algorithm
            )
    ...
    def test_modify_delete_attribute(self):
        ...
        """
        key_id = self.client.create(
            enums.CryptographicAlgorithm.IDEA, 128, name="Symmetric Key"
        )
        ...
        result_id, result_attributes = self.client.get_attributes(
            uid=key_id, attribute_names=["Name"]
        )
        self.assertEqual(1, len(result_attributes))
        self.assertEqual("Name", result_attributes[0].attribute_name.value)
        self.assertEqual(
            "Symmetric Key", result_attributes[0].attribute_value.name_value.value
        )
        ...
        self.assertEqual(
            "Modified Name", response_attr.attribute_value.name_value.value
        )
        ...
        self.assertEqual("Name", result_attributes[0].attribute_name.value)
        self.assertEqual(
            "Modified Name", result_attributes[0].attribute_value.name_value.value
        )

        # Delete the "Name" attribute for the key.
        response_id, response_attr = self.client.delete_attribute(
            unique_identifier=key_id, attribute_name="Name", attribute_index=0
        )
        self.assertEqual(key_id, response_id)
        self.assertEqual("Name", response_attr.attribute_name.value)
        self.assertEqual(0, response_attr.attribute_index.value)
