...
class TestByteString(testtools.TestCase):
    ...
    def test_init(self):
        ...
        self.assertIsInstance(
            bs.value, bytes, self.bad_type.format("value", bytes, type(bs.value))
        )
        self.assertEqual(
            value, bs.value, self.bad_value.format("value", value, bs.value)
        )
    ...
    def test_init_unset(self):
        ...
        self.assertIsInstance(
            bs.value, bytes, self.bad_type.format("value", type(None), type(bs.value))
        )
        self.assertEqual(
            bytes(), bs.value, self.bad_value.format("value", None, bs.value)
        )
    ...
    def test_read_value(self):
        ...
        self.assertEqual(
            expected, bs.value, self.bad_read.format("value", expected, bs.value)
        )
    ...
    def test_read(self):
        encoding = (
            b"\x42\x00\x00\x08\x00\x00\x00\x03\x01\x02\x03\x00\x00\x00\x00" b"\x00"
        )
        self.stream = utils.BytearrayStream(encoding)
        bs = primitives.ByteString()
        bs.read(self.stream)
    ...
    def test_read_on_invalid_padding(self):
        encoding = (
            b"\x42\x00\x00\x08\x00\x00\x00\x03\x01\x02\x03\xff\xff\xff\xff" b"\xff"
        )
        self.stream = utils.BytearrayStream(encoding)
        bs = primitives.ByteString()
