...
class TestBigInteger(testtools.TestCase):
    def setUp(self):
        ...
        self.encoding_zero = utils.BytearrayStream(
            b"\x42\x00\x00\x04\x00\x00\x00\x08\x00\x00\x00\x00\x00\x00\x00" b"\x00"
        )
        ...
        self.encoding_negative = utils.BytearrayStream(
            b"\x42\x00\x00\x04\x00\x00\x00\x08\xFF\xFF\xFF\xFF\xFF\xFF\xFC" b"\x18"
        )
    ...
    def test_read_on_invalid_length(self):
        ...
        big_int = primitives.BigInteger()
        self.assertRaises(
            exceptions.InvalidPrimitiveLength, big_int.read, self.encoding_bad_length
        )
    ...
    def test_repr(self):
        ...
        self.assertEqual("BigInteger({0}, {1})".format(value, tag), repr(long_int))
