...
class TestInteger(testtools.TestCase):
    def setUp(self):
        ...
        self.bad_value = (
            "Bad primitives.Integer.{0} after init: expected {1}, " "received {2}"
        )
        self.bad_write = (
            "Bad primitives.Integer write: expected {0} bytes, " "received {1} bytes"
        )
        self.bad_encoding = "Bad primitives.Integer write: encoding mismatch"
        self.bad_read = "Bad primitives.Integer.value read: expected {0}, received {1}"
    ...
    def test_init(self):
        ...
        self.assertEqual(0, i.value, self.bad_value.format("value", 0, i.value))
        self.assertEqual(
            i.LENGTH, i.length, self.bad_value.format("length", i.LENGTH, i.length)
        )
        self.assertEqual(
            i.LENGTH,
            i.padding_length,
            self.bad_value.format("padding_length", i.LENGTH, i.padding_length),
        )
    ...
    def test_validate_on_invalid_value_too_big(self):
        ...
        """
        self.assertRaises(ValueError, primitives.Integer, primitives.Integer.MAX + 1)

    def test_validate_on_invalid_value_too_small(self):
        """
        Test that a ValueError is thrown on input that is too small.
        """
        self.assertRaises(ValueError, primitives.Integer, primitives.Integer.MIN - 1)
    ...
    def test_read_value_max_positive(self):
        ...
        self.assertEqual(self.max_int, i.value, self.bad_read.format(1, i.value))
    ...
    def test_read(self):
        encoding = (
            b"\x42\x00\x00\x02\x00\x00\x00\x04\x00\x00\x00\x01\x00\x00\x00" b"\x00"
        )
        self.stream = utils.BytearrayStream(encoding)
        i = primitives.Integer()
        i.read(self.stream)
    ...
    def test_read_on_invalid_length(self):
        encoding = (
            b"\x42\x00\x00\x02\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00" b"\x00"
        )
        self.stream = utils.BytearrayStream(encoding)
        i = primitives.Integer()
    ...
    def test_read_on_invalid_padding(self):
        encoding = (
            b"\x42\x00\x00\x02\x00\x00\x00\x04\x00\x00\x00\x00\xff\xff\xff" b"\xff"
        )
        self.stream = utils.BytearrayStream(encoding)
        i = primitives.Integer()
    ...
    def test_write_value(self):
        ...
        self.assertEqual(len_exp, len_rcv, self.bad_write.format(len_exp, len_rcv))
