    ...
    with client.ProxyKmipClient(config=config, config_file=opts.config_file) as client:
        ...
        try:
            key_id = client.create(
                ...
                128,
                cryptographic_usage_mask=[enums.CryptographicUsageMask.DERIVE_KEY],
            )
        ...
        try:
            secret_id = client.derive_key(
                ...
                enums.DerivationMethod.HMAC,
                {
                    ...
                    "derivation_data": (
                        b"\xF0\xF1\xF2\xF3\xF4\xF5\xF6\xF7" b"\xF8\xF9"
                    ),
                    "salt": (
                        b"\x00\x01\x02\x03\x04\x05\x06\x07" b"\x08\x09\x0A\x0B\x0C"
                    ),
                },
        ...
        try:
            ...
            logger.info("Successfully derived a new secret via NIST 800 108-C.")
            logger.info("Secret ID: {0}".format(secret_id))
        except Exception as e:
            logger.error(e)
