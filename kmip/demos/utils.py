...
def build_cli_parser(operation):
    ...
    parser = optparse.OptionParser(
        usage="%prog [options]", description="Run KMIP client operation"
    )
    ...
    if operation is Operation.CREATE:
        ...
        parser.add_option(
            ...
            dest="format",
            help=(
                "Format in which to retrieve the secret. Supported formats "
        ...
        parser.add_option(
            ...
            help="List of attribute names to retrieve, defaults to all " "attributes",
        )
        ...
        parser.add_option(
            ...
            dest="object_type",
            help=(
                "The object type of the secret " "(e.g., CERTIFICATE, SYMMETRIC_KEY)"
            ),
        )
        parser.add_option(
            "--certificate-type",
        ...
        parser.add_option(
            ...
            dest="format",
            help=(
                "Format in which to store the secret. Supported formats "
                "include: RAW, PKCS_1, PKCS_8, X_509"
            ),
        )
        parser.add_option(
            ...
            dest="type",
            help=(
                "Type of the object to register. Supported types include: "
...
def build_cryptographic_usage_mask(logger, object_type):
    if object_type == ObjectType.CERTIFICATE:
        ...
        object_type == ObjectType.SYMMETRIC_KEY or object_type == ObjectType.SECRET_DATA
    ):
        flags = [CryptographicUsageMask.ENCRYPT, CryptographicUsageMask.DECRYPT]
        ...
        logger.error(
            "Unrecognized object type, could not build cryptographic " "usage mask"
        )
        sys.exit()
...
def build_object(logger, object_type, key_format_type):
    ...
    if object_type == ObjectType.CERTIFICATE:
        ...
        return Certificate(
            certificate_type=CertificateType.X_509, certificate_value=value
        )
    else:
...
def build_key(logger, object_type, key_format_type):
    ...
    cryptographic_algorithm = build_cryptographic_algorithm(logger, object_type)
    cryptographic_length = build_cryptographic_length(logger, object_type)

    key_block = build_key_block(
        key_format_type, key_value, cryptographic_algorithm, cryptographic_length
    )
...
def build_cryptographic_length(logger, object_type):
    if object_type == ObjectType.SYMMETRIC_KEY or object_type == ObjectType.SECRET_DATA:
        ...
        logger.error(
            "Unrecognized object type, could not build cryptographic " "length"
        )
        sys.exit()
...
def build_cryptographic_algorithm(logger, object_type):
    if object_type == ObjectType.SYMMETRIC_KEY or object_type == ObjectType.SECRET_DATA:
        ...
        logger.error(
            "Unrecognized object type, could not build cryptographic " "algorithm"
        )
        sys.exit()
...
def log_attribute_list(logger, attributes):
    ...
    for i in range(len(attributes)):
        ...
        logger.info("   attribute_value: {0}".format(repr(attribute_value)))
...
def log_certificate(logger, certificate):
    ...
    logger.info("* certificate type: {0}".format(certificate.certificate_type))
    logger.info(
        "* certificate value: {0}".format(
...
def log_split_key(logger, split_key):
    ...
    logger.info("* Key Part Identifier: {}".format(split_key.key_part_identifier))
    logger.info("* Split Key Threshold: {}".format(split_key.split_key_threshold))
...
def log_key_block(logger, key_block):
    if key_block is not None:
        ...
        logger.info("  * Key Compression Type: {}".format(key_compression_type))
        logger.info("  * Cryptographic Algorithm: {}".format(cryptographic_algorithm))
        logger.info("  * Cryptographic Length: {}".format(cryptographic_length))
...
def log_key_value(logger, key_value):
    if key_value is not None:
        ...
        logger.info("    * Key Material: {}".format(repr(key_value.key_material)))
    else:
